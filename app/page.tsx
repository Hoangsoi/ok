...
export default function Home() {
  ...
  // Simulate ultra-realistic dynamic live transactions arriving continuously
  useEffect(() => {
    ...
    const addNewTransaction = () => {
      ...
      // Random delay between 1.5s to 3.2s for continuous organic real-time updates
  ...
  // Fetch dynamic configuration set from Admin (/admin) and keep synced
  useEffect(() => {
    const loadConfig = () => {
      fetch('/api/config')
        .then((res) => res.json())
  ...
  const openAction = (title: string) => {
    ...
    // Always get the freshest target URL (from localStorage or React state)
    ...
    if (freshTargetUrl) {
      ...
      // Instantly navigate customer to the new target link set in Admin
  ...
  return (
    <main>
      ...
      {/* 3. HERO BANNER - REDESIGNED MATCHING REFERENCE IMAGE */}
